...
### [LRN-20260331-001] 执行 /understand 前必须检查技能
- **Category**: correction
- **Priority**: critical
- **Status**: promoted
- **Summary**: 收到 /understand 请求时没有调用 Understand-Anything 工具，效果差 30 倍
- **Details**: 用户执行 /understand 分析 GitHub 项目时，我用 find/cat/ls 来"分析"，没有检查有没有专门的分析工具。结果：只读了 200 行 vs 原版 5,910 个节点
- **Suggested Action**: using-superpowers 触发概率从 50% 降到 1%，每次任务前必须检查技能
- **Metadata**: Source: user_feedback, Tags: understand, skill-check, priority



### [LRN-20260331-002] Skill 示例数据必须来自实际运行
- **Category**: best_practice
- **Priority**: high
- **Status**: promoted
- **Summary**: SKILL.md 中的示例必须从实际运行结果中提取，不能手写
- **Details**: 创建 understand-project 技能时，示例数据写了 1,231 文件/5,910 节点，但实际运行是 1,207 文件/5,886 节点。用户发现不一致
- **Suggested Action**: 写 SKILL.md 时先运行代码，把输出复制到文档
- **Metadata**: Source: user_feedback, Tags: skill-creation, documentation, accuracy



### [LRN-20260331-003] 数字必须精确，不能随手写
- **Category**: correction
- **Priority**: high
- **Status**: promoted
- **Summary**: 报告数字时必须精确，不能估算或随手写
- **Details**: 写了 1,231 个文件但实际是 1,207 个（差 24 个）。用户指出后我才承认是写错了
- **Suggested Action**: 报告数字前必须验证，输出实际值
- **Metadata**: Source: user_feedback, Tags: accuracy, verification



### [LRN-20260331-004] 创建 Skill 后必须多次测试
- **Category**: best_practice
- **Priority**: high
- **Status**: active
- **Summary**: 创建 Skill 后至少运行 2 次，确保结果一致
- **Details**: understand-project 技能第一次测试用 --no-functions 参数，结果和完整运行不同。导致示例数据不一致
- **Suggested Action**: 多次测试 + 对比结果 + 确认一致后才报告完成
- **Metadata**: Source: user_feedback, Tags: testing, skill-creation



### [LRN-20260331-005] 不要重复造轮子，先查有没有现成工具
- **Category**: correction
- **Priority**: critical
- **Status**: promoted
- **Summary**: 收到任务时先检查 ~/.openclaw/skills/ 有没有适用的技能
- **Details**: 分析 Hermes Agent 时自己用 find/cat/ls，没有调用 Understand-Anything。创建技能时没有查 skillhub 有没有现成的
- **Suggested Action**: using-superpowers 规则：1% 概率就要查技能
- **Metadata**: Source: user_feedback, Tags: skill-check, avoid-duplication



### [LRN-20260331-006] Hermes Agent 集成差距
- **Category**: knowledge_gap
- **Priority**: high
- **Status**: active
- **Summary**: 我的 Hermes Agent 集成只覆盖了 12-68%，差距巨大
- **Details**: 技能自动发现 8%，自改进学习闭环 16%，MCP Client 85%，Context Compressor 70%，Command Approval 50%
- **Suggested Action**: 继续完善：YAML Frontmatter、LLM 总结、RPC 工具调用
- **Metadata**: Source: analysis, Tags: hermes, integration, gap



### [LRN-20260331-007] Harness Engineering 最佳实践
- **Category**: best_practice
- **Priority**: medium
- **Status**: promoted
- **Summary**: 长时间运行 Agent 需要 Harness 结构：Initializer + Coding Agent + Progress File
- **Details**: 参考 Anthropic 文章，使用 progress.txt 和 features.json 在会话间交接状态
- **Suggested Action**: 使用 harness-engineering 技能追踪复杂任务
- **Metadata**: Source: research, Tags: harness, long-running, multi-session



### [LRN-20260331-008] 沉淀经验 → 自动触发 self-improvement
- **Category**: correction
- **Priority**: high
- **Status**: active
- **Summary**: 用户说"沉淀经验"时自动使用 self-improvement skill
- **Details**: 一条明明确要求：以后说"沉淀经验"就使用 self-improving-agent 技能
- **Suggested Action**: 记录到 TOOLS.md 作为自动触发词
